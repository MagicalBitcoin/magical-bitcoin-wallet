...
static MIGRATIONS: &'static [&'static str] = &[
    ...
    "CREATE INDEX idx_indices_keychain ON last_derivation_indices(keychain);",
...
impl SqliteDatabase {
    ...
    pub fn insert_script_pubkey(&self, keychain: String, child: u32, script: &[u8]) -> Result<i64, Error> {
    ...
    pub fn insert_utxo(&self, value: u64, keychain: String, vout: u32, txid: &[u8], script: &[u8]) -> Result<i64,Error> {
    ...
    pub fn insert_transaction(&self, txid: &[u8], raw_tx: &[u8]) -> Result<i64,Error> {
        let mut statement = self.connection.prepare_cached("INSERT INTO transactions (txid, raw_tx) VALUES (:txid, :raw_tx)")?;
        statement.execute(
            named_params!{ 
                ":txid": txid,
                ":raw_tx": raw_tx,
    ...
    pub fn update_transaction(&self, txid: &[u8], raw_tx: &[u8]) -> Result<(),Error> {
        let mut statement = self.connection.prepare_cached("UPDATE transactions SET raw_tx=:raw_tx WHERE txid=:txid")?;
    ...
    pub fn insert_transaction_details(&self, txid: &[u8], timestamp: Option<u64>, received: u64, sent: u64, fee: Option<u64>, height: Option<u32>) -> Result<i64,Error> {
    ...
    pub fn update_transaction_details(&self, txid: &[u8], timestamp: Option<u64>, received: u64, sent: u64, fee: Option<u64>, height: Option<u32>) -> Result<(), Error> {
    ...
    pub fn insert_last_derivation_index(&self, keychain: String, value: u32) -> Result<i64,Error> {
        let mut statement = self.connection.prepare_cached("INSERT INTO last_derivation_indices (keychain, value) VALUES (:keychain, :value)")?;
    ...
    pub fn insert_checksum(&self, keychain: String, checksum: &[u8]) -> Result<i64,Error> {
        let mut statement = self.connection.prepare_cached("INSERT INTO checksums (keychain, checksum) VALUES (:keychain, :checksum)")?;
        statement.execute(
            named_params!{
                ":keychain": keychain,
                ":checksum": checksum,
    ...
    pub fn update_last_derivation_index(&self, keychain: String, value: u32) -> Result<(),Error> {
        let mut statement = self.connection.prepare_cached("UPDATE last_derivation_indices SET value=:value WHERE keychain=:keychain")?;
    ...
    pub fn select_script_pubkeys(&self) -> Result<Vec<Script>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT script FROM script_pubkeys")?;
    ...
    pub fn select_script_pubkeys_by_keychain(&self, keychain: String) -> Result<Vec<Script>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT script FROM script_pubkeys WHERE keychain=:keychain")?;
        let mut scripts: Vec<Script> = vec![];
    ...
    pub fn select_script_pubkey_by_path(&self, keychain: String, child: u32) -> Result<Option<Script>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT script FROM script_pubkeys WHERE keychain=:keychain AND child=:child")?;
    ...
    pub fn select_script_pubkey_by_script(&self, script: &[u8]) -> Result<Option<(KeychainKind, u32)>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT keychain, child FROM script_pubkeys WHERE script=:script")?;
    ...
    pub fn select_utxos(&self) -> Result<Vec<LocalUtxo>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT value, keychain, vout, txid, script FROM utxos")?;
        ...
        while let Some(row) = rows.next()? {
            ...
            utxos.push(LocalUtxo {
                ...
                txout: TxOut { value, script_pubkey: script.into() },
                keychain
            })
        }
    ...
    pub fn select_utxo_by_outpoint(&self, txid: &[u8], vout: u32) -> Result<Option<(u64, KeychainKind, Script)>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT value, keychain, script FROM utxos WHERE txid=:txid AND vout=:vout")?;
    ...
    pub fn select_transactions(&self) -> Result<Vec<Transaction>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT raw_tx FROM transactions")?;
    ...
    pub fn select_transaction_by_txid(&self, txid: &[u8]) -> Result<Option<Transaction>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT raw_tx FROM transactions WHERE txid=:txid")?;
    ...
    pub fn select_transaction_details_with_raw(&self) -> Result<Vec<TransactionDetails>, Error> {
    ...
    pub fn select_transaction_details(&self) -> Result<Vec<TransactionDetails>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT txid, timestamp, received, sent, fee, height FROM transaction_details")?;
        let mut transaction_details: Vec<TransactionDetails> = vec![];
        let mut rows = statement.query([])?;
        while let Some(row) = rows.next()? {
            let txid: Vec<u8> = row.get(0)?;
    ...
    pub fn select_transaction_details_by_txid(&self, txid: &[u8]) -> Result<Option<TransactionDetails>, Error> {
    ...
    pub fn select_last_deriviation_index_by_keychain(&self, keychain: String) -> Result<Option<u32>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT value FROM last_derivation_indices WHERE keychain=:keychain")?;
        let mut rows = statement.query(named_params!{":keychain": keychain})?;
        match rows.next()? {
            Some(row) => {
    ...
    pub fn select_checksum_by_keychain(&self, keychain: String) -> Result<Option<Vec<u8>>, Error> {
        let mut statement = self.connection.prepare_cached("SELECT checksum FROM checksums WHERE keychain=:keychain")?;
        let mut rows = statement.query(named_params!{":keychain": keychain})?;
    ...
    pub fn delete_script_pubkey_by_path(&self, keychain: String, child: u32) -> Result<(), Error> {
        let mut statement = self.connection.prepare_cached("DELETE FROM script_pubkeys WHERE keychain=:keychain AND child=:child")?;
        statement.execute(
            named_params!{
                ":keychain": keychain,
                ":child": child
    ...
    pub fn delete_script_pubkey_by_script(&self, script: &[u8]) -> Result<(), Error> {
        let mut statement = self.connection.prepare_cached("DELETE FROM script_pubkeys WHERE script=:script")?;
        statement.execute(
            named_params!{
                ":script": script
            }
    ...
    pub fn delete_utxo_by_outpoint(&self, txid: &[u8], vout: u32) -> Result<(), Error> {
        let mut statement = self.connection.prepare_cached("DELETE FROM utxos WHERE txid=:txid AND vout=:vout")?;
        statement.execute(
            named_params!{
                ":txid": txid,
                ":vout": vout
    ...
    pub fn delete_transaction_by_txid(&self, txid: &[u8]) -> Result<(), Error> {
        let mut statement = self.connection.prepare_cached("DELETE FROM transactions WHERE txid=:txid")?;
        statement.execute(named_params!{":txid": txid})?;
        Ok(())
    }

    pub fn delete_transaction_details_by_txid(&self, txid: &[u8]) -> Result<(), Error> {
        let mut statement = self.connection.prepare_cached("DELETE FROM transaction_details WHERE txid=:txid")?;
        statement.execute(named_params!{":txid": txid})?;
        Ok(())
    }

    pub fn delete_last_derivation_index_by_keychain(&self, keychain: String) -> Result<(), Error> {
        let mut statement = self.connection.prepare_cached("DELETE FROM last_derivation_indices WHERE keychain=:keychain")?;
        statement.execute(
            named_params!{
                ":keychain": &keychain
            }
...
impl BatchOperations for SqliteDatabase {
    ...
    fn set_raw_tx(&mut self, transaction: &Transaction) -> Result<(), Error> {
        match self.select_transaction_by_txid(&transaction.txid())? {
            Some(_) => {
                // should this ever really happen? wouldn't an adjusted tx
                // result in a new txid
    ...
    fn set_tx(&mut self, transaction: &TransactionDetails) -> Result<(), Error> {
        let (timestamp, height) = match &transaction.confirmation_time {
            Some(confirmation_time) => {
                (Some(confirmation_time.timestamp), Some(confirmation_time.height))
            },
            None => {
                (None, None)
            }
        ...
        match self.select_transaction_details_by_txid(&transaction.txid)? {
            ...
            None => {
                self.insert_transaction_details(
                    &transaction.txid, 
                    timestamp,
                    transaction.received,
                    transaction.sent,
    ...
    fn del_last_index(&mut self, keychain: KeychainKind) -> Result<Option<u32>, Error> {
        let keychain = serde_json::to_string(&keychain)?;
        match self.select_last_deriviation_index_by_keychain(keychain.clone())? {
            Some(value) => {
...
impl Database for SqliteDatabase {
    ...
    fn get_tx(&self, txid: &Txid, _include_raw: bool) -> Result<Option<TransactionDetails>, Error> {
        // always returns with Option<Transaction> (if exists in db)
        // are there cases where we NEED it to be None even if it exists in db?
        // or is this just a performance improvement?
        match self.select_transaction_details_by_txid(&txid)? {
            Some(transaction_details) => {
                Ok(Some(transaction_details))
            },
    ...
    fn get_last_index(&self, keychain: KeychainKind) -> Result<Option<u32>, Error> {
        let keychain = serde_json::to_string(&keychain)?;
        match self.select_last_deriviation_index_by_keychain(keychain)? {
            Some(value) => {
                Ok(Some(value))
            },
            None => {
...
pub fn set_schema_version(conn: &Connection, version: i32) -> rusqlite::Result<usize> {
    conn.execute("UPDATE version SET version=:version", named_params!{":version": version})
}
